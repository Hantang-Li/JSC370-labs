...
Here we observed that these three stations does not coincide.
...
median_temp_station_loc <- met %>%  filter(USAFID == median_temp_station$USAFID)
median_wind.sp_station_loc <- met %>%  filter(USAFID == median_wind.sp_station$USAFID)
median_atm.press_station_loc <- met %>%  filter(USAFID == median_atm.press_station$USAFID)
...
leaflet() %>% 
  ...
  addCircles(data = median_temp_station_loc[1],
             lat = ~lat, lng = ~lon, popup="median_temp_station",
             opacity = 1, fillOpacity = 1, radius = 400, color = "Red") %>% 
  addCircles(data = median_wind.sp_station_loc[1],
             lat = ~lat, lng = ~lon, popup="median_wind.sp_station",
             opacity = 1, fillOpacity = 1, radius = 400, color = "Yellow")%>% 
  addCircles(data = median_atm.press_station_loc[1],
             lat = ~lat, lng = ~lon, popup="median_atm.press_station",
...
#station_ave[,atm.press_50 := quantile(atm.press, probs=.5, na.rm=T), by=STATE]
...
```{r}
#gam_mod <- gam(temp_50 ~ s(wind.sp_50, bs = "cr", k=20), data=station_ave)# 20 knocs 
gam_mod <- gam(temp_50 ~ s(wind.sp_50, bs = "cr"), data= station_ave)
summary(gam_mod)
plot(gam_mod)
```

The spline model is the best fit because the line model has adjusted r-square approximately equal to 0.17 which is terrible, which shows not much variance of the data is being explained by the model. However, the adjusted r-square of the spline model is 0.417, it is much higher than 0.17 and shows more variance is being explained by the spline model.

Further, the visualized linear model does not closely match the data's distribution, but the visualized spline model matches the distribution of the data closer compared to the linear model.
...
https://github.com/Hantang-Li/JSC370-labs
